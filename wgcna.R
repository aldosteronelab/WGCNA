...
data=data[apply(data,1,sd)>0.5,]   #删除波动小的基因


#读取样品信息

samp <- read.table('smaple0.txt', sep = '\t', header = T)

sample1 <- data.frame(samp[samp$APA == 0, 1])

sample2 <- data.frame(samp[samp$APA == 1, 1])
